...
const project: IProject.Payload = {
  ...
  list: [
    {
      title: 'We:Jet API 백엔드 스크립트 번들러 구축',
      startedAt: '2021-04',
      endedAt: '2021-05',
      ...
      descriptions: [
        {
          content:
            'We:Jet은 Cafe24와 같은 타사의 쇼핑몰 플랫폼에 솔루션을 삽입해 컴포넌트와 위젯 등을 더하거나 뺄 수 있는 프로젝트입니다.'
        },
        {
          content:
            '타사의 솔루션들은 최초 스크립트를 페이지에 삽입시키고 이후 XHR 요청 후 해당하는 플러그인을 추가로 삽입하는 형태로 구현이 되어 있었습니다만, 회선 상태에 따라 TTFB가 길어져 사이트 로딩과 트래픽을 크게 늘리는 요인으로 작용하였습니다'
        },
        {
          content:
            'ES Module로 번들링시 Tree-Shaking이 되는 점에 착안해 We:Jet에서 쓰이는 모든 Plugin과 Component를 사전 빌드 및 번들링하고, 쇼핑몰 삽입 스크립트 엔드포인트 호출 시 DB에서 쇼핑몰에 쓰일 컴포넌트와 플러그인 목록을 가져와 선택적으로 번들링하도록 구현하였습니다.'
        },
        {
          content:
            '그 결과 개별 플러그인 형태로 퍼블리싱하는 것보다 JS 및 API 요청이 유의미적으로 줄어들고 번들링으로 인한 파일 크기 감소로 initialize까지 약 < 100ms, 트래픽이 70% 가량 감소하는 성과를 얻었습니다'
        }
      ]
    },
    {
